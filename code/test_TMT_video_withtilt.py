...
def temp_segment(total_frames, trunk_len, valid_len):
    ...
    for i in range(1, num_chunk+1):
        if i == num_chunk:
            test_frame_info.append({'start':total_frames-12, 'range':[i*valid_len+residual,total_frames]})
...
if args.march == 'normal':
    model = RT_warp(num_blocks=[2,3,3,4], num_refinement_blocks=2, n_frames=12).to(device)
elif args.march == 'FAtt':
    model = RT_warp(num_blocks=[2,3,3,4], num_refinement_blocks=2, n_frames=12, att_type='full').to(device)
elif args.march == 'simple':
    model = RT_warp(num_blocks=[2,3,3,4], num_refinement_blocks=2, n_frames=12, att_type='simple').to(device)
elif args.march == 'group':
    model = RT_warp(num_blocks=[2,3,3,4], num_refinement_blocks=2, n_frames=12, att_type='group').to(device)
elif args.march == 'shuffle':
    model = RT_warp(num_blocks=[2,3,3,4], num_refinement_blocks=2, n_frames=12, att_type='shuffle').to(device)
elif args.march == 'normal_nores':
    model = RT_warp(num_blocks=[2,3,3,4], num_refinement_blocks=2, n_frames=12, out_residual=False).to(device)
elif args.march == 'warpenc':
    model = RT_warp(num_blocks=[2,3,3,4], num_refinement_blocks=2, n_frames=12, warp_mode='enc').to(device)
elif args.march == 'warpdec':
    model = RT_warp(num_blocks=[2,3,3,4], num_refinement_blocks=2, n_frames=12, warp_mode='dec').to(device)
elif args.march == 'warpall':
    model = RT_warp(num_blocks=[2,3,3,4], num_refinement_blocks=2, n_frames=12, warp_mode='all').to(device)
...
with torch.no_grad():
    ...
    for data in test_loader:
        ...
        if os.path.exists(img_result_path):
            time.sleep(1)
            continue
        num_chunk, test_frame_info = temp_segment(total_frames, 12, 10)
    ...
    with open(f'{result_dir}/result.log', 'a') as log_file:
        log_file.write(f'Overall psnr:{sum(psnr)/len(psnr)}, ssim:{sum(ssim)/len(ssim)}, psnry:{sum(psnry)/len(psnry)}, ssimy:{sum(ssimy)/len(ssimy)}')
