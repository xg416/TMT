...
test_dataset = DataLoaderTurbImageTest(rgb_dir=input_dir, num_frames=48, total_frames=50)
test_loader = DataLoader(dataset=test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=12, drop_last=False, pin_memory=True)
...
with torch.no_grad():
    ...
    for data in test_loader:
        ...
        for i in range(args.total_frames//args.temp_window):
            ...
            recovered = test_spatial_overlap(input_.permute(0,2,1,3,4), model, model_tilt, patch_size).permute(0,2,1,3,4)
            if iteration > 1:
                for iter in range(iteration-1):
                    recovered = test_spatial_overlap(recovered.permute(0,2,1,3,4), model, model_tilt, patch_size).permute(0,2,1,3,4)
            ...
            ssim_y_img = []
            lpips_img = []
            # img_gt = gt[b, ...].data.squeeze().float().cpu().clamp_(0, 1).numpy()
            # if img_gt.ndim == 3:
